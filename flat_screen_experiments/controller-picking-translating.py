...
# It requires getObjectInfoRay function https://github.com/FreeCAD/FreeCAD/pull/16789/commits
# In this example a coin shape (loaded from *.iv file) representing controller is moved by openvr input
# Save the "right_con.iv" file in the FreeCAD bin directory, otherwise a
# simple cube will be used as fallback
...
class Tracker(object):
    def __init__(self):
        ...
        # this separator contains everything that moves with controller/tracker
        self.tracker_sep = coin.SoSeparator()
    ...
    def add_ray_node(self):
        ...
        # Set first vertex, later update to center of the controller
    ...
    def pick_ray(self, pos, rot, dir_vec):
        ...
        if (info):  # FIXME script will fail if picked point lost during draggin
            ...
            end_ray = coin.SbVec3f(sect_pt.x, sect_pt.y, sect_pt.z)
            ...
            App.Console.PrintMessage(
                "Trigger " + str(self.trigger_state) + "\n")
    ...
    def update_controller(self, pos, rot, state):
        ...
        # //direction is reversed controller Z get_ray_axis
        self.pick_ray(pos, rot, - self.get_ray_axis(rot))
    def read_file(self, filename):
        # Open the input file
        scene_input = coin.SoInput()
        if not scene_input.openFile(filename):
            App.Console.PrintMessage(
                "Cannot open file:" + str(filename) + "\n")
            return None
        # Read the whole file into the database
        graph = coin.SoDB.readAll(scene_input)
        if graph is None:
            App.Console.PrintMessage(
                "Problem reading file" + str(filename) + "\n")
            return None
    ...
    def extract_coin_rotation(self, transfmat):  # extract rotation quaternion
        qw = math.sqrt(
            np.fmax(
                0,
                1 + transfmat[0][0] + transfmat[1][1] + transfmat[2][2])) / 2
        qx = math.sqrt(
            np.fmax(
                0,
                1 + transfmat[0][0] - transfmat[1][1] - transfmat[2][2])) / 2
